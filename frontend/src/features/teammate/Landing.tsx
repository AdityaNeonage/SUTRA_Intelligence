...
const PIPELINE_STEPS = [
  ...
  { icon: '🧠', label: 'Evidence Extraction', sub: 'Supported document extraction', color: '#d946ef' },
...
const FEATURES = [
  ...
  },
  {
    ...
    color: '#22d3ee',
    items: [
      { title: 'Evidence Hub', desc: 'Upload supported files, track processing and inspect stored records.' },
      { title: 'Multimodal AI', desc: 'Live extraction supports CSV, text, PDF and DOCX. Image, video and audio analysis are illustrative.' },
...
function Header({ onOpenDashboard, onOpenLogin }: { onOpenDashboard: () => void; onOpenLogin: () => void }) {
    ...
    <header className={cn('fixed top-0 left-0 right-0 z-40 transition-all duration-300 border-b', scrolled ? 'bg-[#0a0710]/95 backdrop-blur border-white/10' : 'bg-transparent border-transparent')}>
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="hidden md:inline text-[8px] text-white/30 tracking-widest uppercase px-2 py-0.5 border border-white/10">Prototype</span>
        </div>
        <nav className="flex items-center gap-3 md:gap-6 text-[9px] uppercase tracking-widest text-white/40">
          <a href="#features" className="hidden md:block hover:text-white transition-colors">Features</a>
          <a href="#pipeline" className="hidden md:block hover:text-white transition-colors">Pipeline</a>
          <a href="#copilot" className="hidden md:block hover:text-white transition-colors">Copilot</a>
          ...
          <button onClick={onOpenLogin} className="px-4 py-2 bg-white text-black font-bold">Sign in</button>
        </nav>
      </div>
    </header>
  );
      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        ...
        >
          ...
          <p className="text-white/40 text-sm max-w-xl mx-auto">
            Supported uploads are stored, parsed and linked where recognised fields permit. Inspect processing warnings and source records before interpreting connections.
          </p>
        </motion.div>
...
function GhostNodeSection({ onOpenDashboard }: { onOpenDashboard: () => void }) {
  return (
    <section className="py-28 bg-[#0a0710]">
      <div className="max-w-6xl mx-auto px-6">
        <div className="grid md:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <h2 className="text-4xl md:text-5xl font-black text-white uppercase tracking-tight mb-6">Explore What's Missing</h2>
            <p className="text-white/40 text-sm leading-relaxed mb-8">
              Explore an illustrative structural-gap hypothesis. This sample shows how an investigator could review missing links; it is not a live prediction or confirmed fact.
            </p>
            ...
            <div className="border border-[#d946ef]/25 bg-[#d946ef]/5 p-4">
              ...
              <div className="flex items-center justify-between mt-3 pt-3 border-t border-white/5">
                ...
                <div className="flex gap-2">
                  <button onClick={onOpenDashboard} className="px-3 py-1 bg-[#d946ef] text-black text-[7px] font-black uppercase tracking-widest hover:bg-[#d946ef]/80 transition-colors">Investigate</button>
                  <button onClick={onOpenDashboard} className="px-3 py-1 border border-white/10 text-white/40 text-[7px] font-bold uppercase tracking-widest hover:text-white transition-colors">Dismiss</button>
                </div>
              </div>
            </div>
          </motion.div>
              ...
              <div className="flex gap-3">
                ...
                <div className="border-l-2 border-[#d946ef] border-y-white/5 border-r-white/5 bg-sutra-dark px-5 py-4 flex-1">
                  ...
                  <div className="flex items-center justify-between mb-4 p-2 border border-[#d946ef]/15 bg-[#d946ef]/5">
                    <span className="text-[8px] text-white/40 uppercase tracking-widest">Illustrative confidence</span>
...
function Footer({ onOpenDashboard }: { onOpenDashboard: () => void }) {
  return (
      ...
      <div className="max-w-7xl mx-auto px-6">
          ...
          <div>
            ...
            <ul className="space-y-2.5 text-[8px] uppercase tracking-widest text-white/30">
              {['Investigation Graph', 'Evidence Hub', 'SUTRA Copilot', 'AI Insights', 'Timeline'].map(l => (
                <li key={l}><button onClick={onOpenDashboard} className="hover:text-white transition-colors">{l}</button></li>
              ))}
            </ul>
          </div>
          <div>
            <h4 className="text-[8px] font-black uppercase tracking-widest text-white mb-4">Prototype boundaries</h4>
            <ul className="space-y-2.5 text-[8px] uppercase tracking-widest text-white/30">
              {['Use synthetic evidence only', 'Review every inference', 'No automated guilt decisions'].map(l => (
        ...
        <div className="border-t border-white/5 pt-6 flex items-center justify-between text-[7px] uppercase tracking-widest text-white/20">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span>Investigator prototype</span>
          </div>
        </div>
      </div>
    </footer>
...
export default function Landing({ onOpenDashboard, onOpenLogin }: { onOpenDashboard: () => void; onOpenLogin: () => void }) {
  return (
    <div className="team-ui team-landing min-h-screen bg-[#0a0710] font-sans text-white selection:bg-[#d946ef]/30 selection:text-white">
      <Header onOpenDashboard={onOpenDashboard} onOpenLogin={onOpenLogin} />
      <main>
        <Hero onOpenDashboard={onOpenDashboard} />
        <div className="team-prototype-note">Prototype preview · Examples below are synthetic. Live uploads support CSV, JSON, TXT, PDF and DOCX; media analysis and predictive AI shown here are demonstrations, not live capabilities.</div>
        <PipelineSection />
        <VehicleDemo />
        <CrossEvidenceSection />
        <GhostNodeSection onOpenDashboard={onOpenDashboard} />
