...
export function DataStorePage({ token, initialCaseId, initialIngestionId, initialDocumentId }: { token: string; initialCaseId?: string; initialIngestionId?: string; initialDocumentId?: string }) {
  const cases = useCases(token);
  const [caseId, setCaseId] = useState(initialCaseId ?? "");
  const [ingestionId, setIngestionId] = useState(initialIngestionId ?? "");
  const requestId = useRef(0);
  useEffect(() => { if (initialDocumentId) void inspect(initialDocumentId); return () => { requestId.current++; }; }, [initialDocumentId]);
  ...
  useEffect(() => {
    if (ingestionId && ingestions.data && !ingestions.data.some((item) => item.id === ingestionId)) setIngestionId("");
  ...
  async function inspect(documentId: string) {
    ...
    setDetailError("");
    try {
      const data = await sutraApi.getDocument(token, documentId);
      if (request === requestId.current) setSelected(data);
    } catch (error) {
      if (request === requestId.current) setDetailError(errorMessage(error));
    } finally {
      if (request === requestId.current) setDetailLoading(false);
    }
  }

  const activeBatch = ingestions.data?.find(item => item.id === ingestionId);
  function clearDetail() { requestId.current++; setSelected(undefined); setDetailLoading(false); setDetailError(""); }
  ...
  return (
    <div className="page-stack data-store-page">
      ...
      <section className="panel data-store-toolbar">
        <label>Case<select className="input" value={caseId} onChange={(event) => { setCaseId(event.target.value); setIngestionId(""); clearDetail(); }}><option value="">All accessible data</option>{cases.data?.map((item) => <option key={item.id} value={item.id}>{item.case_number}</option>)}</select></label>
        <label>Upload batch<select className="input" value={ingestionId} onChange={(event) => { setIngestionId(event.target.value); clearDetail(); }}><option value="">All batches</option>{ingestions.data?.map((item) => <option key={item.id} value={item.id}>{item.filename} · {item.id.slice(0, 8)}</option>)}</select></label>
      ...
      {activeBatch && <section className="panel intake-body"><h3>{activeBatch.filename}</h3><p>{activeBatch.status} · {activeBatch.source_type} · {activeBatch.document_count} stored records</p><p className="intake-help">SHA-256: <code style={{ overflowWrap: "anywhere" }}>{activeBatch.file_hash}</code></p>{activeBatch.error_message && <p className="inline-error">{activeBatch.error_message}</p>}</section>}
      <p className="intake-help">Lists show up to 200 records or batches. Use case, batch and search filters to narrow the results.</p>
