...
export default function CrimeHotspotMap() {
    ...
    useEffect(() => {
        if (document.getElementById("leaflet-css-sutra")) {
            setLeafletReady(true);
            return;
        }
        const link = document.createElement("link");
        link.id = "leaflet-css-sutra";
        link.rel = "stylesheet";
    ...
    useEffect(() => {
        ...
        import("leaflet").then((L) => {
            ...
            // UI UPGRADE: Implemented CartoDB Dark Matter Base Map
           L.tileLayer(
            ...
            // Set state to trigger the marker building effect immediately!
            setMapInstance(map);
        });
    ...
    // Rebuild markers when filters change OR when map initially loads
    useEffect(() => {
        if (!mapInstance) return; // Wait until map is injected into state
        ...
        import("leaflet").then((L) => {
            ...
            // Build new markers based on current active selection
            buildMarkers(L, mapInstance, mapMarkersData);

            // Automatically fit the map to show all currently visible markers
            if (mapMarkersData.length > 0) {
                const bounds = L.latLngBounds(mapMarkersData.map(c => [c.lat, c.lng]));
                mapInstance.flyToBounds(bounds, { padding: [50, 50], duration: 0.8, maxZoom: 15 });
            }
        });
    }, [mapMarkersData, mapInstance]); // Dependency array updated
    ...
    return (
      ...
      `}</style>
            ...
            <div style={{ display: "flex", height: "100%", width: "100%", background: "#060608" }}>
